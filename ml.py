...
class ValueModel:
    def __init__(self, players: pd.DataFrame):
        ...
        # Beyond raw attributes, three columns carry real signal about value:
        # overall_ability summarises quality in one number, height matters for
        # some positions, and current wage is a strong market signal (clubs pay
        # for what they rate). Adding them lifts cross-val R2 ~0.70 -> ~0.73.
        extra = [c for c in ("overall_ability", "height_cm", "salary_eur")
                 if c in players.columns]
        self.features = self.attrs + ["Age", "league_enc"] + extra
        self._train(players)
        self._score_all(players)
        self._calibrate_bargain_thresholds(players)

    def _calibrate_bargain_thresholds(self, players: pd.DataFrame):
        """Set bargain / overpriced cutoffs on the predicted-to-actual ratio.

        A player is flagged a BARGAIN when the model values him at 1.5x his
        listed price or more (predicted at least 50% above market), and overpriced
        at the mirror point of ~0.67x. A fixed, intuitive multiple is clearer to
        read than a shifting percentile and matches how scouts talk about value.
        """
        self.bargain_hi = 1.5
        self.bargain_lo = round(1 / 1.5, 2)   # ~0.67
    ...
    def _train(self, players: pd.DataFrame):
        ...
        # XGBoost (tuned) outperforms GradientBoosting on this data. With the
        # extra numeric features (wage, overall ability, height) cross-val R2 is
        # ~0.73 on FM26 — a little below FM23's ~0.78 only because FM26 spans far
        # more leagues with noisier value labels. Restricting training to the top
        # leagues was tried and does NOT help (held-out R2 is unchanged and it
        # hurts predictions for smaller-league players — the hidden gems), so the
        # model is trained on the full population. Fall back to sklearn if
        # xgboost isn't installed.
        if _HAS_XGB:
            # tree_method='hist' with 300 trees trains in ~4s instead of ~8s for
            # 600 at the default method, for effectively identical accuracy
            # (cross-val R2 0.729 vs 0.731) — worth it for a snappier startup.
    ...
    def predict(self, players: pd.DataFrame) -> pd.Series:
        # An empty pool (all candidates filtered out) would make the model
        # return a 0x0 array and crash the Series constructor, so short-circuit.
        if len(players) == 0:
    ...
    def bargain_ratio(self, players: pd.DataFrame) -> pd.Series:
        """predicted / actual. Compare against self.bargain_hi / self.bargain_lo
        (calibrated to the loaded dataset) to flag bargains vs overpriced."""
    ...
    def feature_importance(self, top: int = 8) -> list[tuple[str, float]]:
        names = {**ATTRIBUTES, "Age": "Age", "league_enc": "League level",
                 "overall_ability": "Overall ability", "height_cm": "Height",
                 "salary_eur": "Current wage"}
